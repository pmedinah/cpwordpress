...
class Sinatra_Core_Custom_List_Widget extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $new_instance['items'] ) ) {
			foreach ( $new_instance['items'] as $entry ) {
				...
				$new_entry = array(
					'icon'        => isset( $entry['icon'] ) ? sanitize_text_field( $entry['icon'] ) : '',
						...
						<div class="si-repeatable-item">
							...
							<div class="si-repeatable-item-content">
								...
								<p>
									...
									<input type="text" class="widefat si-icon-picker-input" id="<?php echo $this->get_field_id( 'items' ) . '-' . $index . '-icon'; ?>" name="<?php echo $this->get_field_name( 'items' ); ?>[<?php echo $index; ?>][icon]" value="<?php echo $entry['icon']; ?>" placeholder="si-icon si-map-pin"/>
								</p>
	...
	function dynamic_styles( $css ) {
		$css .= '.sinatra-core-custom-list-widget .si-widget-icon {
